...
Laser Maze adds ten finish-based routes and an endless climb: keep the ball inside a red laser corridor built from right-angled legs, climbing a column and crossing sideways before climbing again, up to the checkered line. Touching a wall ends the run. The routes grow narrower and add crossings, with local best times per route and control scheme; endless generates the corridor forever, tightens as it goes, and scores the height climbed. All existing controls remain available; one-finger automatic ascent waits under a sideways leg instead of driving into it.
